!async function () {
    document.title = "Danberg 1.0 - By Tikup#8555";
    const intercom = new BroadcastChannel("danberg");
    ...
    const
        ...
        nav = (function () {
            ...
            const doalilwork = function () { // "we do a lil work"
                ...
                if (navdata.isMaster == 1) intercom.postMessage([0, navdata.coords]);
    ...
    const dan = {
        ...
        mode: (mode) => intercom.postMessage([1, mode]),
        target: () => intercom.postMessage[2, navdata.coords],
        tolerance: (t) => (navdata.tolerance = t, intercom.postMessage[2, navdata.tolerance]),
    ...
    intercom.onmessage = function (e) {
        if (navdata.isMaster == 1) return;

        const message = e.data,
            type = message[0];
        switch (type) {
            case 0: // master coord update
                navdata.master = message[1];
                break;

            case 1: // set mode
                navdata.mode = message[1];
                break;

            case 2: // set target/home
                navdata.target = message[1];

            case 3: // set tolerance
                navdata.tolerance = message[1];
        }
    }
